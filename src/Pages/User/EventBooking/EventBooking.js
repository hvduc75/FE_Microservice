...
import { createBooking, getBookingByEventId } from '../../../service/bookingService';
...
function EventBooking(props) {
    ...
    const fetchBooking = async () => {
        let data = await getBookingByEventId(eventId);
        if (data.EC === 0) {
            setBooking(data.DT);
            if(data.DT) {
                setShowModal(true);
    ...
    return (
        <>
            ...
            {showModal && <ModalConfirmBooking booking={booking} setShowModal={setShowModal} />}
        </>
    );
}
